...
const PasswordCriteria = ({ criteria }: { criteria: { [key: string]: boolean } }) => {
  const criteriaText: { [key: string]: string } = {
    minLength: "At least 8 characters", uppercase: "Contains an uppercase letter", lowercase: "Contains a lowercase letter",
    number: "Contains a number", specialChar: "Contains a special character",
  };
  return (
      ...
      <ul className="text-sm space-y-2">
        {Object.entries(criteria).map(([key, value]) => (
          <li key={key} className="flex items-center">
            ...
            <span className={`strike-through ${value ? 'active text-gray-500' : 'text-gray-300'}`}>{criteriaText[key]}</span>
          </li>
        ))}
      </ul>
    </div>
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 200) { toast.success("OTP sent to your email!"); router.push(`/otp?email=${form.email}`); } else { toast.error(res.data.detail || "Signup failed"); }
    } catch (err: unknown) {
      if (axios.isAxiosError(err) && err.response?.data?.detail) { toast.error(err.response.data.detail); } else { toast.error("An unexpected error occurred during signup."); }
  ...
  const handleGoogleSignup = async (credentialResponse: CredentialResponse) => {
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/google`, { token: credentialResponse.credential });
      const { access_token, user_id, is_first_time_user } = res.data;
      localStorage.setItem("token", access_token); localStorage.setItem("user_id", user_id);
      Cookies.set("token", access_token, { expires: 1 }); Cookies.set("user_id", user_id, { expires: 1 });
      toast.success("Google signup successful!"); setRedirectPath(is_first_time_user ? "/preferences?new_user=true" : "/dashboard"); setIsSuccess(true);
    } catch { toast.error("Google signup failed"); setLoading(false); }
  };

  const handleAnimationFinish = () => { if (isSuccess) router.replace(redirectPath); };
