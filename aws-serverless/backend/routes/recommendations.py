...
async def generate_recommendations(current_user: dict = Depends(get_current_user)):
    ...
    if is_pro_user(user) and user.get("sheets_enabled"):
        print(f"--- Attempting to write recommendations to Google Sheet for user {user_id} ---")
        ...
        if success:
            ...
            sheets_error = f"Failed to write to Google Sheet for user {user_id}. Check backend logs for details."
            print(f"--- {sheets_error} ---")
